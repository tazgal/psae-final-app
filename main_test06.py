...
######## 
########  Περίοδος ενασχόλησης 26/2/2026 - 
...
# Προσπάθεια για ανέβασμα διαδικτυακά
...
# ============================================================
from mod_08_geo import * 

# ============================================================
# 📁 MODULE 09 — Timelines
# ============================================================
from mod_09_timelines4 import *

# ============================================================
# 📁 MODULE 10 — Relation extraction
# ============================================================

from mod_10_relation_extraction import *
...
################### ECONOMY TOOLS ###################
    ...
    econ_tab1, econ_tab2, econ_tab3, econ_tab4, econ_tab5 = st.tabs(
        ["Working Area","Open Pdfs","Data","Charts","Tools"])
    ...
    with econ_tab3:
        ...
        with col1:
            ...
            if st.session_state["data"] == "excel":
        ...
        with col2:
            ...
            if st.button("Read excel/csv"):
                    st.session_state["data"] = "excel"
    ...
    with econ_tab4:
            ...
            with col1:
                ...
                if st.session_state["charts"] == "open":
                    search_images2()
...
########################
...
elif main_menu == "NLP - Visualization":

    nlp_tab1, nlp_tab2, nlp_tab3, nlp_tab4, nlp_tab5 = st.tabs(["NLP","Relation Viz","Timelines","Maps","Dictionaries"]) 
    ...
    with nlp_tab1:
        ...
        with col1:
            ...
            # Persons
            if st.session_state["nlp"] == "similarity":
                s_col1, s_col2 = st.columns(2)
                with s_col1:
                    text1 = st.text_area("Κείμενο 1", height=400, key="text1")
                with s_col2:
                    text2 = st.text_area("Κείμενο 2", height=400, key="text2")
                ...
                user_text = st.text_area("Please enter your text here to analyze")
                

                if  st.session_state["nlp"] == "persons":
                    if user_text.strip():
                        results = nlp_spacy_ner_grouped_entities3(user_text)
                ...
                # Dates
                elif st.session_state["nlp"] == "dates":
                    if user_text.strip():
                ...
                # Highlight
                elif st.session_state["nlp"] == "highlight":
                    if user_text.strip():
                        nlp_highlight(user_text)
                    else:
                        st.warning("Γράψε πρώτα κείμενο!")

                # Voyant
                elif st.session_state["nlp"] == "voyant":
                    st.components.v1.iframe(
                ...
                # Concordance
                elif st.session_state["nlp"] == "corc":
                    keyword = st.text_input("Enter keyword")
                    window = st.slider("Contextual size", 2, 5, 10)
                    if user_text.strip() and keyword:
                        tokens = word_tokenize(user_text)
                        df = concordance_table(tokens, keyword, window)
                        st.subheader(f"Concordances για '{keyword}'")
                        st.dataframe(df)

            
        with col2:

            if st.button("Highlight",help="Uses SpaCy to highlight entities"):
                st.session_state["nlp"] = "highlight"
                st.session_state["text_input"] = True

            if st.button("Persons and Organizations",help="Uses SpaCy to find entities"):
                st.session_state["nlp"] = "persons"
                st.session_state["text_input"] = True

            if st.button("Find dates",help="Uses regex to find dates"):
                st.session_state["nlp"] = "dates"
                st.session_state["text_input"] = True

            if st.button("Similarity",help="Uses SpaCy to highlight entities"):
                st.session_state["nlp"] = "similarity"
                st.session_state["text_input"] = False

            if st.button("Concordances",help="Uses Nltk to show corcondances"):
                st.session_state["nlp"] = "corc"
                st.session_state["text_input"] = True
            
            if st.button("Voyant",help="Opens Voyant tools Online"):
                st.session_state["nlp"] = "voyant"
                st.session_state["text_input"] = False
    ...
    with nlp_tab2:
        ...
        if "rel_extr" not in st.session_state:
            st.session_state["rel_extr"] = "diagram"
        if "text_input" not in st.session_state:
            st.session_state["text_input"] = True
        ...
        with col1:
            if st.session_state["rel_extr"] == "diagram":
                create_graph2("data/csvs")

            if st.session_state["rel_extr"] == "diagram2":
                create_graph_N_edges("data/csvs")

            if st.session_state["rel_extr"] == "diagram3":
                pass

            if st.session_state["rel_extr"] == "companies":
                companies_data = get_data("data/lexika/companies.json")
                map_network2(companies_data)


        with col2:

            if st.button("Relation extraction",help="Makes a diagramm of a xls with pyvis"):
                st.session_state["rel_extr"] = "diagram"

            if st.button("More than 2",help="Vizualize a diagramm of a xls with pyvis"):
                st.session_state["rel_extr"] = "diagram2"

            if st.button("Show companies relations",help="Show graph for companies"):
                st.session_state["rel_extr"] = "companies"

            if st.button("with json",help="Create gpaph from json"):
                st.session_state["rel_extr"] = "diagram3"
    ...
    with nlp_tab3:
        ...
        if "timeline" not in st.session_state:
            st.session_state["timeline"] = "create_line"

        col1, col2 = st.columns(([4,1]), gap="small", vertical_alignment="top", border=True)

        with col2:
            if st.button("Create Timeline",help="Creates a timeline from csv"):
                st.session_state["timeline"] = "create_line"

            if st.button("Gantt Timeline",help="Creates a Gantt timeline from csv"):
                st.session_state["timeline"] = "gantt"

            

        with col1:
            if st.session_state["timeline"] == "create_line":
                horizontal_timeline_app3()
            
            if st.session_state["timeline"] == "gantt":
                gant_timeline_app(folder="data/csvs")
    ...
    with nlp_tab4:
        ...
        # Init session_state για tab και text
        if "map" not in st.session_state:
            st.session_state["map"] = "clorepath"
        ...
        with col1:
            ...
            # Επιλογή λειτουργίας
            if st.session_state["map"] == "create_map":
                map_text = st.text_area("Please enter your text",height=300)
                scr_from_text_to_map_streamlit2(map_text)
            
            elif st.session_state["map"] == "clorepath":
                map_text = st.text_area("Please enter your text",height=300)
                choropleth_from_text7(map_text)

            elif st.session_state["map"] == "map_tools":
                with open("data/maps/map_tools.json", encoding="utf-8") as f:
                    resources = json.load(f)

                st.subheader("🗺️ Geospatial & Mapping Tools")
                tool = st.selectbox("Επίλεξε εργαλείο", list(resources.keys()))
                url = resources[tool]
                st.markdown(f"- [{tool}]({url})", unsafe_allow_html=True)
                st.components.v1.iframe(url, height=600, scrolling=True)


        with col2:
            if st.button("Show Capitals on map"):
                st.session_state["map"] = "create_map"

            if st.button("Create countries clorepath"):
                st.session_state["map"] = "clorepath"
            
            if st.button("Map Tools"):
                st.session_state["map"] = "map_tools"
    ...
    with nlp_tab5:
        ...
        if "dict" not in st.session_state:
            st.session_state["dict"] = "semantics"
        ...
        with col1:
            ...
            if st.session_state["dict"] == "semantics":
                lex_semantics_geo(user_text)
            
            if st.session_state["dict"] == "my dict":
                st.markdown("#### Τα link από δικό μου λεξικό")
                lex_add_links_exoteriko(user_text)

            if st.session_state["dict"] == "custom":
                lexiko_custom_show(user_text)

            if st.session_state["dict"] == "economy":
                lex_oikonomikon(user_text)

        
        with col2: 

            if st.button("Link to geo terms semantics.gr"):
                st.session_state["dict"] = "semantics"
                
            if st.button("Add links",help="Adds links to a custom dictionary"):
                st.session_state["dict"] = "my dict"
                
            if st.button("Highlight terms", help="Highlight custom terms on a text"):
                st.session_state["dict"] = "custom"
            
            if st.button("Economy terms",help="terms from taxhorizon"):
                st.session_state["dict"] = "economy"
