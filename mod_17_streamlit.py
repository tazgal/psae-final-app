...
from mod_15_GPTs import gpt_deepseek_streamlit_short, gpt_deepseek_vasika_simeia
...
def st_read_news():
    ...
    search_4 = ['ΗΠΑ', 'Τράμπ', 'Γερμανία', "Γαλλία", "Ιταλία", "ευρωζώνη", "Ρωσία", "Κίνα", "ΝΑΤΟ", "ΕΕ", "Ευρωπαϊκή Ένωση", "εμπορικός πόλεμος", "δασμοί"]
            ...
            if response.status_code == 200:
                ...
                for t in titles:
                    ...
                    if any(keyword.lower() in text.lower() for keyword in keywords):
                        ...
                        title_all.append({
                            "Τίτλος": text,
                            "Σύνδεσμος": href if href else url  # αν δεν υπάρχει link, κράτα το γενικό
                        })
                        news_urls.append(href)
        except Exception as e:
            st.warning(f"Πρόβλημα με {url}: {e}")

    
    print(news_urls)
    return pd.DataFrame(title_all), news_urls
...
st.markdown("# News toolbox (test)")

main_menu = option_menu(
    "Κύριο Μενού",
    ["Ριζοσπάστης - 902", "Πρωτοσέλιδα", "Ειδήσεις", "Αποδελτίωση", "Scrap Τίτλοι", "Tools", "Σημειωματάριο", "Ημερολόγιο"],
    icons=["house", "newspaper", "globe", "calendar-event","database","tools","journal-text","calendar"],
    menu_icon="cast",
    default_index=0
)
