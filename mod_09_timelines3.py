...
# --- ΡΥΘΜΙΣΗ ΜΗΚΟΥΣ ΓΡΑΜΜΩΝ ΓΙΑ ΚΑΘΕΤΟ TIMELINE ---
if layout_choice == "Κάθετη":
    line_length_percentage = st.slider(
        "📏 Μήκος οριζόντιων γραμμών (%)",
        min_value=10,
        max_value=100,
        value=70,  # Προεπιλεγμένη τιμή 70%
        help="Ρύθμισε το μήκος των οριζόντιων γραμμών που συνδέουν τις βούλες με το κείμενο"
    )
    # Μετατροπή από ποσοστό σε δεκαδικό
    line_multiplier = line_length_percentage / 100.0
else:
    line_multiplier = 1.0  # Προεπιλογή για οριζόντιο timeline
...
if layout_choice == "Οριζόντια":
    ...
    # Κρατάμε τις αρχικές θέσεις για βούλες και κείμενο
    original_offsets = offsets.copy()
    
    # Συνδέσεις (γραμμές) και βούλες με τις προσαρμοσμένες θέσεις
    for i, (y, off) in enumerate(zip(range(len(df)), original_offsets)):
        event = df["Γεγονός"].iloc[i]
        date = df["Χρονολογία"].iloc[i]
        
        # Γραμμή σύνδεσης - ΧΡΗΣΙΜΟΠΟΙΕΙ ΤΟ line_multiplier ΜΟΝΟ ΕΔΩ
        fig.add_shape(
            type="line",
            x0=0, x1=off * line_multiplier,  # <-- ΜΟΝΟ οι γραμμές πολλαπλασιάζονται
            y0=y, y1=y,
            line=dict(color="gray", width=1)
        )
        
        # Προσθήκη βούλας - ΧΡΗΣΙΜΟΠΟΙΕΙ ΤΗΝ ΑΡΧΙΚΗ ΘΕΣΗ
        fig.add_trace(go.Scatter(
            x=[off], y=[y],  # <-- Αρχική θέση για βούλα
            mode="markers",
            marker=dict(size=marker_size, color="royalblue"),
            showlegend=False,
            hoverinfo="text",
        ...
        # Προσθήκη κειμένου με ημερομηνία - ΧΡΗΣΙΜΟΠΟΙΕΙ ΤΗΝ ΑΡΧΙΚΗ ΘΕΣΗ
        fig.add_annotation(
            x=off, y=y,  # <-- Αρχική θέση για κείμενο
            text=f"<b>{date}</b><br>{event}",
            showarrow=False,
    ...
    fig.update_layout(
        ...
        title=f"Κάθετο Timeline (Μήκος γραμμών: {line_length_percentage}%)"
    )
